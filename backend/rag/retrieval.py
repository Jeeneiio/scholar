...
class Retriever:
    ...
    def retrieve(
        ...
        """
        # 主流程：子块召回 →（可选）重排 → 父块扩展 → 去重 → Top-K。
        # 每一步在速度、精度和给 LLM 的上下文完整性之间折中。
        ...
        # HyDE 用“假想论文段落”改善语义召回；正常业务路径默认关闭，避免额外 LLM 调用。
        ...
        if rerank and self.reranker:
            # 先宽召回，再让 CrossEncoder 对 (query, 文档) 成对打分；
            # 比单独比较 embedding 更精确，但成本更高，因此只处理候选集。
        ...
        if expand_parent:
            # 命中的是小子块，返回时恢复其父块，避免生成模型只看到半段上下文。
    ...
    def _hybrid_search(
        ...
        # Milvus Lite currently cannot execute the server-side FunctionScore
        # reranker used for BM25+dense RRF. Fall back to dense search locally;
        # the full hybrid path remains enabled for Milvus Standalone/Server.
        if not self.milvus_uri.startswith(("http://", "https://")):
            kwargs = {"k": k}
            if expr:
                kwargs["expr"] = expr
            logger.warning("Milvus Lite detected; using dense-only retrieval instead of hybrid RRF")
    ...
    def _rerank(self, query: str, docs: list[Document], top_k: int) -> list[Document]:
        if not docs:
            return docs
        # CrossEncoder 同时看到问题和候选文本，输出的是“这段文本能否回答问题”的相关性分数。
    ...
    def _expand_to_parents(self, children: list[Document]) -> list[Document]:
        # 子块的 chunk_parent_id 指向完整节点的 chunk_id；保持顺序去重，
        # 避免同一段落的多个相邻子块反复返回同一个父块。
