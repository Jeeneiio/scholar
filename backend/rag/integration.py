...
class PDFParser:
    ...
    def __init__(self, figure_save_dir: Optional[Path] = None, llm=None, enable_section_classification: bool = True):
        self.cleaner = TextCleaner()
        self._converter_cache = {}
        self.figure_save_dir = figure_save_dir or FIGURE_SAVE_DIR
        self.llm = llm
        self.enable_section_classification = enable_section_classification
    ...
    def parse(self, pdf_path: str, paper_id: str) -> list[PaperNode]:
        ...
        # 扫描版 PDF 常常提取不到文本；以文本总量和每页密度判断是否需要 OCR 兜底。
    ...
    def _parse_with_ocr(self, pdf_path: str, paper_id: str, use_ocr: bool) -> list[PaperNode]:
        ...
        # 为图表补上 caption、将正文中的 Figure/Table 引用关联到对应节点，
        # 并裁出原图供后续 VLM 按需读取。
...
class RAGIntegration:
    ...
    def nodes_to_documents(self, nodes: list[PaperNode], content_hash: str = "") -> list[Document]:
        ...
        for node in nodes:
            ...
            # `item` 是 Docling 的运行时对象，不能可靠地序列化进向量库；
            # 其余 metadata 则是引用、筛选和多模态问答所需的溯源信息。
    ...
    def create_chunks(self, docs: list[Document]) -> tuple[list[Document], list[Document]]:
        ...
        # 这些节点拆开会破坏原子语义：表格会丢列关系，图会脱离图注，标题/图注通常很短。
        ...
        for doc in docs:
            ...
            if should_split:
                # 子块更小，适合 embedding 检索；50 字符重叠降低句子在边界被截断的影响。
    ...
    def store_in_milvus(self, parents: list[Document], children: list[Document]) -> bool:
        ...
        try:
            ...
            # 父集合通过 chunk_id 与子块的 chunk_parent_id 关联。
