...
def _build_sub_agent_graph(
    ...
    # 每个子问题独立走一条 RAG 闭环；反思不通过时最多重试 max_retries 次。
...
def build_graph(
    ...
    def dispatch(state: AgentState):
        # LangGraph Send 将拆分出的子问题并发派给同一个子图，之后由 state 的
        # 聚合规则收集 sub_answers，供最终综合回答使用。
    ...
    # 主图先压缩会话记忆，再给问题路由和拆分；最终只由 synthesize 输出用户可见答案。
