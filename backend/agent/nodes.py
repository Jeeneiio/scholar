...
def _language_instruction(query: str) -> str:
    """Tell the model which language to use for the final answer.

    Retrieved paper passages are often in English, so relying on the context
    language can make an answer switch languages unexpectedly.
    """
    chinese_chars = len(re.findall(r"[\u4e00-\u9fff]", query))
    latin_chars = len(re.findall(r"[A-Za-z]", query))
    if chinese_chars > 0 and chinese_chars >= latin_chars * 0.1:
        return (
            "# Response Language\n"
            "Answer in Simplified Chinese because the user's question is in Chinese. "
            "Keep technical terms, model names, equations, and citations unchanged when appropriate. "
            "Do not switch to English merely because the source passages are in English."
        )
    if latin_chars > 0:
        return (
            "# Response Language\n"
            "Answer in the same language as the user's question. "
            "Do not switch languages merely because the source passages use another language."
        )
    return (
        "# Response Language\n"
        "Answer in the same language as the user's question."
...
async def analyze_query(state: AgentState, llm: BaseChatModel) -> dict:
    ...
    # 用结构化输出约束 LLM 返回可调度的子问题列表，而不是自由文本。
...
async def classify_query(state: AgentState, llm: BaseChatModel) -> dict:
    ...
    # 分类结果会映射为 section_type 过滤器，例如方法问题优先检索 method 章节。
...
def prepare_synthesis(state: AgentState) -> dict:
    ...
    system_content += f"\n\n{_language_instruction(state['query'])}"
    if context_header:
        system_content += f"\n\n# Conversation Context\n{context_header}"
...
async def retrieve(state: SubAgentState, retriever, citation_extractor) -> dict:
    ...
    # 一次反思可能生成多个补充检索词；并发执行以降低重试的总等待时间。
    ...
    # 引用从 Document metadata 提取，因此能保留论文、章节、页码等可核验来源。
...
async def generate(state: SubAgentState, llm: BaseChatModel, vision_service=None) -> dict:
    ...
    # 不把“没有检索到证据”伪装成正常回答，这是减少 RAG 幻觉的第一道边界。
    if not documents:
        return {"answer": "No relevant information found.", "citations": []}

    # 图表问答时，文本 caption 可能不含曲线趋势或视觉数值；按需增加 VLM 描述。
...
async def reflect(state: SubAgentState, llm: BaseChatModel, vision_service=None) -> dict:
    ...
    # 反思只判断是否缺失“关键部分”；不足时产生更针对性的下一轮检索词。
    ...
    # VLM fallback: 文本回答不足、召回中有图且尚未分析时，最多分析两张图控制成本。
